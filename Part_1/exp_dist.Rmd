...
```
The resulting distribution is plotted below. Also depicted are the population mean, $\mu =$ `r round(exp_mean,2)`, and population standard deviation, $\sigma =$ `r round(exp_sd,2)`.
...
x <- data.frame(Sample=c(round(mns_mean,2), round(sd(mns$values), 2)), Theoretical=c(mu, round(se, 2)), Exponential=c(round(exp_mean,2), round(exp_sd, 2)), row.names=c("Mean", "SD"))
